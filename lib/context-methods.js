 ...
 */
async function step(desc, takeScreenshot = true) {
    ...
    const { I } = context
    if (takeScreenshot) await I.say(desc)

    // Record steps only in tests
    // console.log(this._test)
    if (this._test.metadata.type !== 'test') return
    ...
    for (step of report.outline.steps) {
        if(step.success === undefined) {
            step.reachedAt = Date.now()
            step.success = true
            step.actualName = desc
            break;
        }
    }
}
