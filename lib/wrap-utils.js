...
const isAutoScreenshotFunction = (I, fnName) => {
  ...
  : ['amOnPage', 'switchTo', 'click', 'see', 'seeTextEquals', 'say', 'seeElement', 'seeInField', 'seeInTitle', 'seeNumberOfVisibleElements', 'dontSee', 'dontSeeElement', 'dontSeeInField'].indexOf(fnName) > -1
    ...
    try {
        if (!isAppium(I)) { // highlight elements only in browser tests
          ...
          if (isAssertionFn(fn) && isSelector(I, selectorFromArgs)) {
              try {
                  await I._highlightElement(selectorFromArgs, `I ${fn.name} ${args.map(a => `${a}`).join(',') || ''}`, isError)
...
const correctStackLines = stackLines => stackLines
  ...
  . filter(l => l.indexOf('next_tick.js') === -1)
