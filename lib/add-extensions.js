...
/**
 * My version of codeceptjs waitForVisible
 * 
 * Because I noticed that chromedriver might sometimes return false
 * in elementIdDisplayed although the element actually is visible
 */
function _waitForVisible(locator, sec) {
    ...
    return client.waitUntil(function () {
      return client.elements(withStrictLocator(locator)).then(function (res) {
        ...
        let commands = [];
        const el = res.value[0]
        commands.push(client.elementIdDisplayed(el.ELEMENT))
        commands.push(client.elementIdEnabled(el.ELEMENT))
        commands.push(client.elementIdCssProperty(el.ELEMENT, 'display'))
        commands.push(client.elementIdCssProperty(el.ELEMENT, 'visibility'))
        ...
        return client.unify(commands, {
          extractValue: true
            ...
            const [displayed, enabled, display, visibility] = result
            console.log(result)
            
            return displayed || (enabled && display !== 'none' && visibility !== 'hidden')
        });
      });
