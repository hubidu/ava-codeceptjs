...
const { extractStepOutlineFromSource } = require('./extract-step-outline-from-source')

test('it should find the test in the test file source', () => {
  const testFnSource = `async ({step}, I) => {
    const newData = {
      street: 'Altostr.',
      houseNumber: '12a',
      areaCode: '81245',
  ...
  const completeSource = `const { test } = require('ava');
  ...
  const res = extractStepOutlineFromSource(completeSource, testFnSource)
