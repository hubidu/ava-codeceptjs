...
const grabBrowserLogsSafe = async actor => {
    ...
    try {
        ...
        const perflogs = await actor.browser.log('performance')
        const browserlogs = await actor.browser.log('browser')
        console.log(browserlogs)
        return perflogs.value.filter(l => l.level === 'SEVERE').concat(browserlogs.value)
    } catch (err) {
