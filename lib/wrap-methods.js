...
const isAssertionFn = fn => fn.name.indexOf('see') === 0

const isSelector = txt =>
  txt &&
  typeof txt === 'string' &&
  (txt.indexOf('#') >= 0 || txt.indexOf('.') >= 0 || txt.indexOf('>') >= 0 || txt.indexOf('=') >= 0 || txt.indexOf('//') === 0 || txt.indexOf('android') === 0)
...
const takeScreenshot = async (I, fn, args, isError = false) => {
    try {
        if (!isAppium(I)) { // highlight elements only in browser tests
          ...
          if (isAssertionFn(fn) && isSelector(selectorFromArgs)) {
              try {
...
function wrapFn (actor, fn) {
    return async function wrapped(...args) {
        ...
        try {
             ...
             */
            const selectorFromArgs = grabSelectorFromArgs(fn.name, args)
            if (isAutowaitFunction(I, fn.name) && isSelector(selectorFromArgs)
            ) {
                try {
