...
const isSelector = txt =>
  txt && typeof txt === 'string' && (txt.indexOf('#') >= 0 || txt.indexOf('.') >= 0 || txt.indexOf('>') >= 0 || txt.indexOf('=') || txt.indexOf('//') || txt.indexOf('android') >= 0)
...
const isAutowaitFunction = (I, fnName) => {
  if (isAppium(I)) {
    return ['click', 'see', 'seeElement', 'seeNumberOfVisibleElements', 'grabHTMLFrom', 'grabTextFrom', 'switchTo'].indexOf(fnName) > -1
  } else {
    return ['click', 'see', 'seeElement', 'seeNumberOfVisibleElements', 'grabHTMLFrom', 'grabTextFrom', 'fillField', 'switchTo'].indexOf(fnName) > -1
  }
}
...
function wrapFn (actor, fn) {
    return async function wrapped(...args) {
        ...
        try {
             ...
             */
            if (isAutowaitFunction(I, fn.name) && isSelector(grabSelectorFromArgs(fn.name, args))
            ) {
                try {
                    const waitForInSecs = I.options.waitForTimeout
                    debug(`Autowaiting on ${fn.name} for element ${grabSelectorFromArgs(fn.name, args)} (${waitForInSecs})`, )

                    await I._autoWait(grabSelectorFromArgs(fn.name, args), waitForInSecs)
                    // await I.browser.waitForVisible(grabSelectorFromArgs(fn.name, args), 10000)
                } catch (err) {
                    // Just ignore that to avoid failing on StaleElementReferenceException
                    console.log(`WARNING Autowait for element ${grabSelectorFromArgs(fn.name, args)} failed`, err)
                }
            }

            // Execute the step
            debug('CMD', fn.name, args)
