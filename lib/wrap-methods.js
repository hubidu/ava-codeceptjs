...
const isAutoScreenshotFunction = (I, fnName) => {
  ...
  : ['click', 'see', 'seeTextEquals', 'say', 'seeElement', 'seeInField', 'seeInTitle', 'seeNumberOfVisibleElements', 'dontSee', 'dontSeeElement', 'dontSeeInField'].indexOf(fnName) > -1

  return res
}

const isAutowaitFunction = fnName => ['click', 'see', 'seeElement', 'seeNumberOfVisibleElements', 'grabHTMLFrom', 'grabTextFrom', 'fillField', 'switchTo'].indexOf(fnName) > -1
    ...
    return async function wrapped(...args) {
        ...
        try {
            ...
            if (isAutowaitFunction(fn.name) && isSelector(grabSelectorFromArgs(fn.name, args))
                ...
                try {
                    debug(`Autowaiting on ${fn.name} for element`, grabSelectorFromArgs(fn.name, args))

                    await I._autoWait(grabSelectorFromArgs(fn.name, args), I.waitForTimeout / 1000)
